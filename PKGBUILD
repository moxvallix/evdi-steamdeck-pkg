...
pkgname='evdi-steamdeck'
_pkgname='evdi-git'
pkgver=1.12.0.r0.g9c3683e
_pkgver="${pkgver%%.r*}"
pkgrel=1
...
package() {
  cd "${_srcdir}"
  install -Dpm755 "library/lib${_pkgname%-git}.so"* -t "${pkgdir}/usr/lib/"
  ...
  local _DKMS="${pkgdir}/usr/src/${_pkgname%-git}-${_pkgver}"
